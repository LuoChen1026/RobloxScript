if getgenv().DRB_WINDUI_ACTIVE then
...
getgenv().DRB_WINDUI_ACTIVE = true
...
local MeleeSection = CombatTab:Section({ Title = "近战杀戮光环" })

MeleeSection:Toggle({
	Title = "细剑杀戮光环",
	Flag = "RapierKillAuraToggle",
	Default = false,
	Callback = function(Value)
		RapierKillAura.Enabled = Value
		if Value then RKA_start() end
	end,
})

MeleeSection:Slider({
	Title = "细剑攻击范围",
	Flag = "RapierKillAuraRange",
	Step = 1,
	Value = { Min = 5, Max = 100, Default = 30 },
	Callback = function(Value) RapierKillAura.Range = Value end,
})

MeleeSection:Slider({
	Title = "细剑攻击间隔",
	Flag = "RapierKillAuraInterval",
	Step = 0.1,
	Value = { Min = 0.1, Max = 3, Default = 0.5 },
	Callback = function(Value) RapierKillAura.Interval = Value end,
})

MeleeSection:Toggle({
	Title = "细剑 · 自动转向",
	Flag = "RapierAutoRotate",
	Default = false,
	Callback = function(Value) RapierKillAura.AutoRotate = Value end,
})

MeleeSection:Toggle({
	Title = "细剑 · 自动装备",
	Flag = "RapierAutoEquip",
	Default = false,
	Callback = function(Value) RapierKillAura.AutoEquip = Value end,
})
...
MeleeSection:Toggle({
	Title = "近战杀戮光环",
	Flag = "MeleeKillAuraToggle",
	Default = false,
	Callback = function(Value)
		MeleeKillAura.Enabled = Value
		if Value then MKA_start() end
	end,
})

MeleeSection:Slider({
	Title = "近战攻击范围",
	Flag = "MeleeKillAuraRange",
	Step = 1,
	Value = { Min = 5, Max = 100, Default = 30 },
	Callback = function(Value) MeleeKillAura.Range = Value end,
})

MeleeSection:Slider({
	Title = "近战攻击间隔",
	Flag = "MeleeKillAuraInterval",
	Step = 0.1,
	Value = { Min = 0.1, Max = 3, Default = 0.5 },
	Callback = function(Value) MeleeKillAura.Interval = Value end,
})

MeleeSection:Toggle({
	Title = "近战 · 自动转向",
	Flag = "MeleeAutoRotate",
	Default = false,
	Callback = function(Value) MeleeKillAura.AutoRotate = Value end,
})
...
MeleeSection:Toggle({
	...
	Flag = "MeleeAutoEquip",
	Default = false,
	Callback = function(Value) MeleeKillAura.AutoEquip = Value end,
})

local RifleSection = CombatTab:Section({ Title = "步枪杀戮光环" })

RifleSection:Toggle({
	Title = "步枪杀戮光环",
	Flag = "RifleKillAuraToggle",
	Default = false,
	Callback = function(Value)
		RifleKillAura.Enabled = Value
		if Value then RKA_rifle_start() end
	end,
})

RifleSection:Slider({
	Title = "步枪攻击范围",
	Flag = "RifleKillAuraRange",
	Step = 1,
	Value = { Min = 10, Max = 1000, Default = 300 },
	Callback = function(Value) RifleKillAura.Range = Value end,
})

RifleSection:Slider({
	Title = "步枪攻击间隔",
	Flag = "RifleKillAuraInterval",
	Step = 0.01,
	Value = { Min = 0.05, Max = 2, Default = 0.3 },
	Callback = function(Value) RifleKillAura.Interval = Value end,
})

RifleSection:Toggle({
	Title = "步枪 · 自动转向",
	Flag = "RifleAutoRotate",
	Default = false,
	Callback = function(Value) RifleKillAura.AutoRotate = Value end,
})
...
RifleSection:Toggle({
	...
	Flag = "RifleAutoEquip",
	Default = false,
	Callback = function(Value) RifleKillAura.AutoEquip = Value end,
})

RifleSection:Toggle({
	Title = "步枪 · 自动换弹",
	Flag = "RifleAutoReload",
	Default = false,
	Callback = function(Value) RifleKillAura.AutoReload = Value end,
})

RifleSection:Slider({
	Title = "换弹检测间隔",
	Flag = "RifleReloadInterval",
	Step = 0.1,
	Value = { Min = 0.5, Max = 10, Default = 2.5 },
	Callback = function(Value) RifleKillAura.ReloadInterval = Value end,
})
...
for _, name in ipairs(RKA_targetNames) do
	...
	RifleSection:Toggle({
		Title = "攻击 " .. cn,
		Flag = "RifleTarget_" .. name,
		Default = false,
		Callback = function(Value) RifleKillAura.Targets[name] = Value end,
	})
end

local BlockSection = CombatTab:Section({ Title = "格挡与碰撞箱" })
...
BlockSection:Toggle({
	...
	Callback = function(Value)
		...
		if Value then BKA_start() else BKA_stop() end
	end,
})

BlockSection:Dropdown({
	Title = "格挡武器",
	Flag = "AutoBlockWeapon",
	Values = { "Sabre", "Rapier" },
	Value = "Sabre",
	Callback = function(Value)
		AutoBlock.ToolName = Value
		if AutoBlock.Enabled then BKA_stopBlocking(); task.wait(0.1); BKA_checkTool() end
	end,
})

BlockSection:Slider({
	Title = "格挡动画速度",
	Flag = "AutoBlockAnimSpeed",
	Step = 1,
	Value = { Min = 1, Max = 50, Default = 20 },
	Callback = function(Value) AutoBlock.AnimSpeed = Value end,
})
...
BlockSection:Toggle({
	Title = "僵尸受击范围扩大",
	Desc = "在僵尸身上挂透明大盒子，命中更容易",
	Flag = "HitboxModToggle",
	Default = false,
	Callback = function(Value)
...
BlockSection:Slider({
	...
	Flag = "HitboxModScale",
	Step = 0.1,
	Value = { Min = 1.0, Max = 10.0, Default = 4.0 },
	Callback = function(Value) HitboxMod.Scale = Value end,
})

local ESPTab = Window:Tab({ Title = "透视", Icon = "eye" })
local ESPSection = ESPTab:Section({ Title = "身体透视" })
...
local MoveTab = Window:Tab({ Title = "移动", Icon = "move" })

local MoveSection = MoveTab:Section({ Title = "移动" })

MoveSection:Toggle({
	Title = "飞行",
	Desc = "WASD移动，空格上升，左Shift下降",
	Flag = "FlyToggle",
	Default = false,
	Callback = function(Value)
...
local VisSection = MoveTab:Section({ Title = "视觉与动画" })

VisSection:Toggle({
	Title = "动作播放器",
	Desc = "开启后可从下方选择动作播放",
	Flag = "AnimPlayerToggle",
	Default = false,
	Callback = function(Value)
...
local HealTab = Window:Tab({ Title = "治疗", Icon = "heart" })

local HealSection = HealTab:Section({ Title = "全图治疗" })
...
HealSection:Toggle({
	...
	Default = false,
	Callback = function(Value)
		FullMapHeal.Enabled = Value
		if Value then FMH_start() end
	end,
})
...
HealSection:Dropdown({
	Title = "治疗白名单（空=全部）",
	Desc = "选择要治疗的玩家。都不选则治疗所有人",
	Flag = "HealWhitelist",
	Values = (function()
		local t = {}
		for _, p in ipairs(FMH_Players:GetPlayers()) do
			if p ~= FMH_LocalPlayer then
		...
		if type(Value) == "table" then
			for k, v in pairs(Value) do
				if v == true and type(k) == "string" then
					local plr = FMH_Players:FindFirstChild(k)
					if plr then
						newWL[plr] = true
						count = count + 1
					end
...
SelfHealSection:Toggle({
	...
	Default = false,
	Callback = function(Value)
		SelfHeal.Enabled = Value
		if Value then SH_start() else SH_stop() end
	end,
})
...
SelfHealSection:Slider({
	...
	Step = 1,
	Value = { Min = 1, Max = 150, Default = 99 },
...
local UnloadTab = Window:Tab({ Title = "卸载", Icon = "power" })
local UnloadSection = UnloadTab:Section({ Title = "控制" })
...
UnloadSection:Button({
	...
	Callback = function()
		...
		getgenv().DRB_WINDUI_ACTIVE = nil
		task.spawn(function() task.wait(0.1); Window:Destroy() end)
	end,
})
...
print("星火脚本已加载 (WindUI) | 作者星火 群582669853")
