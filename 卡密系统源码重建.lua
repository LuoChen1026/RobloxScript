...
local function ShowLoadingScreen(onComplete)
    ...
    local phases = {}
    local phaseNames = {"正在初始化", "正在创建缓存文件夹", "正在下载资源图标", "正在准备UI面板", "准备就绪"}
...
local function CreateUserInfoPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap, startOpen)
    ...
    copyBtn.MouseButton1Click:Connect(function()
        ...
        Arqel:Notify("复制成功", "HWID已成功复制到剪切板", 2, "copy")
    end)
...
local function handleKeylessSkip()
    ...
    getgenv().ArqelLoaded = false
    Arqel:Notify("验证通过", "免密钥脚本已加载完成!", 3, "success")
    task.wait(0.3)
...
local function BuildKeylessUI()
    ...
    closeBtn.MouseButton1Click:Connect(function()
        Arqel:Notify("再见", "期待下次相遇!", 2, "close")
    ...
    launchBtn.MouseButton1Click:Connect(function()
        Arqel:Notify("加载中", "脚本加载成功!", 2, "success")
        getgenv().SCRIPT_KEY = "KEYLESS"
        getgenv().ArqelLoaded = false
        closeDoorsThenExit(function()
            disableBlur()
    ...
    discordBtn.MouseButton1Click:Connect(function()
        if Arqel.Links.Discord ~= "" then
            Arqel:Notify("Discord", "邀请链接已复制!", 2, "discord")
...
local function BuildKeyUI()
    ...
    textBox.PlaceholderText = "输入你的卡密..."
    ...
    local acquireBtn = createButton(Arqel.Options.NoGetKey and "不可获取" or "获取卡密", Arqel.Options.NoGetKey and "nogetkey" or "key", false, acquireStartY)
    ...
    local redeemBtn = createButton("验证卡密", "shield", true, acquireStartY + buttonHeight + 5)
    ...
    if showShop then
        ...
        buyBtn.MouseButton1Click:Connect(function()
            if Arqel.Shop.Link ~= "" then
                ...
                Arqel:Notify("商店", "商店链接已复制到剪切板!", 2, "copy")
            end
        end)
    end
    ...
    local function handleRedeem()
        ...
        if Internal.ValidateFunction then
            ...
            if success then
                if type(result) == "table" then
                    ...
                    local errMsgs = {
                        KEY_INVALID = "卡密不存在", KEY_EXPIRED = "卡密已过期",
                        HWID_BANNED = "设备已被封禁", KEY_INVALIDATED = "卡密已被取消",
                        ALREADY_USED = "一次性卡密已使用过", HWID_MISMATCH = "设备绑定数量达到上限",
                        SERVICE_NOT_FOUND = "找不到验证服务器", SERVICE_MISMATCH = "服务不匹配",
                        PREMIUM_REQUIRED = "需要高级权限卡密", ERROR = "网络请求出错"
                    }
        ...
        if valid then
            ...
            setStatus("success") Arqel:Notify("成功", "卡密验证已通过!", 2, "success") task.wait(1)
            closeDoorsThenExit(function()
                disableBlur()
                TweenService:Create(container, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {Position = UDim2.new(0.5, 0, -0.5, 0)}):Play()
                TweenService:Create(mainFrame, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
    ...
    acquireBtn.MouseButton1Click:Connect(function()
        if Arqel.Options.NoGetKey then
            Arqel:Notify("不可用", "获取卡密功能暂时不可用", 3, "nogetkey")
            return
        end
        if Arqel.Links.GetKey ~= "" then
            Arqel:Notify("复制成功", "卡密链接已复制!", 3, "copy")
            pcall(function() setclipboard(Arqel.Links.GetKey) end)
        else
            Arqel:Notify("错误", "没有卡密链接部署", 3, "warning")
        end
    end)
    discordBtn.MouseButton1Click:Connect(function()
        if Arqel.Links.Discord ~= "" then Arqel:Notify("Discord", "邀请链接已复制!", 2, "discord") pcall(function() setclipboard(Arqel.Links.Discord) end) end
    end)
...
function Arqel:Launch()
    ...
    if existingKey and existingKey ~= "" then
        if existingKey == "KEYLESS" then
            Arqel:Notify("加载完成", "脚本加载成功!", 2, "success")
    ...
    EnsureIconsReady(function()
        ...
        if Arqel.Storage.AutoLoad and Internal.ValidateFunction then
            ...
            if savedKey and savedKey ~= "" then
                Arqel:Notify("检查中", "正在验证已保存的卡密...", 2, "shield") task.wait(0.5)
                if validateKey(savedKey, Internal.ValidateFunction) then
                    getgenv().SCRIPT_KEY = savedKey
                    Arqel:Notify("欢迎回来", "卡密验证通过!", 2, "success")
                    if Arqel.Callbacks.OnSuccess then Arqel.Callbacks.OnSuccess() end return
                else clearKey() Arqel:Notify("过期", "保存的卡密已失效", 3, "warning") task.wait(1) end
            end
        end
        BuildKeyUI()
        while not getgenv().SCRIPT_KEY do task.wait(0.1) end
...
function Arqel:LaunchJunkie(config)
    ...
    EnsureIconsReady(function()
        ...
        if not success or not Junkie then Arqel:Notify("错误", "Junkie SDK加载失败", 5, "error") return end
        ...
        if Arqel.Storage.AutoLoad then
            ...
            if savedKey and savedKey ~= "" then
                ...
                if vs and vr and vr.valid then
                    ...
                    Arqel:Notify("欢迎回来", "卡密验证通过!", 2, "success")
                    if Arqel.Callbacks.OnSuccess then Arqel.Callbacks.OnSuccess() end return
                else clearKey() Arqel:Notify("已过期", "保存的卡密已失效", 3, "warning") task.wait(1) end
            end
        end
        BuildKeyUI()
        while not getgenv().SCRIPT_KEY do task.wait(0.1) end
